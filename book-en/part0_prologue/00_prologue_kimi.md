...
> 📚 **A small note on chapter numbering**: The book's directory structure on disk uses names like `part0_序章` / `part1_认识这个系统` / `part2_*` / `part3_*` (part0 is the prologue, part1–part5 are the five main content directories). But in the "Reading Paths" below, **Part 1 / Part 2 / ...** are counted from the first body section after the prologue—therefore "Part 3: Curiosity-Driven Deep Q&A" in the narrative actually lives in the `part2_好奇心驱动的深度问答/` directory on disk (adjacent to `part2_代码架构完全解构/`, representing the "Architecture Deconstruction" and "Q&A" halves). This slight naming offset is a historical artifact and does not affect reading—just remember "prologue is part0, subsequent directories start from part1; while the book's narrative Part 1–6 are the logical groupings of the body content."
