...
*质量自检：*
- [x] 覆盖：四大 MemoryType（Managed/User/Project/Local）、@include 指令系统、frontmatter 条件规则、安全边界、yoloClassifier 缓存注入、缓存策略、claudeMdExcludes、HTML 注释剥离、worktree 去重
- [x] 忠实：基于 claudemd.ts、context.ts、memory/types.ts、frontmatterParser.ts、managedPath.ts 源码分析
- [x] 批判：指出全量拼接不确定性、向上遍历 ambient authority、缓存与实时性矛盾、token 爆炸风险、特性开关演进信号
