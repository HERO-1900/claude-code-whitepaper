...
*质量自检：*
- [x] 覆盖：三层架构 + Runtime 推断 + seatbelt 弃用 + 策略格式 + autoAllow 深度分析 + bare git 攻击 + 企业管控 + 平台差异 + 竞品对比
- [x] 忠实：所有行号来自 sandbox-adapter.ts、sandboxTypes.ts、shouldUseSandbox.ts 的实际读取
- [x] 深度：Runtime 层推断分析、autoAllowBashIfSandboxed 信任模型、bare git repo 完整攻击路径、OS 原语 vs 容器化决策分析
- [x] 批判：seatbelt 弃用风险、Runtime 黑盒信任问题、autoAllow 单点信任模型、适配器职责膨胀
- [x] 竞品对比：贯穿全文（Codex、Cursor、E2B、Docker、Copilot Workspace）
